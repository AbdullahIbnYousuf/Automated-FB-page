# Manual Meta Setup — Phases 4 and 5

Phase 4 verifies one Facebook Page with a read-only Graph API request. Phase 5 can schedule exactly one caption and one JPEG/PNG when both backend publishing switches are intentionally enabled. It does not publish immediately, edit, cancel, or delete Facebook content.
...
- Scheduling permission: `pages_manage_posts`; Meta enforces it on the Phase 5 write

Meta's current permission reference lists `pages_read_engagement` and `pages_show_list` as the Page read/discovery permissions. Meta identifies `pages_manage_posts` for Page post creation.
...
4. Open Meta's [Graph API Explorer](https://developers.facebook.com/tools/explorer/), select that app, and generate a User access token with `pages_show_list`, `pages_read_engagement`, and `pages_manage_posts`. Do not request comments, messages, ads, analytics, or Instagram permissions.
...
9. Confirm that the returned Page name and ID are the intended Page. A read-only connection success proves identity, not write permission; only a controlled successful Phase 5 write proves scheduling capability.

## Phase 5 scheduled-photo contract

- endpoint: `POST /v26.0/{page-id}/photos`
- multipart image field: `source`
- text fields: `caption`, `published=false`, `scheduled_publish_time=<Unix seconds>`, `unpublished_content_type=SCHEDULED`
- current scheduling window: at least 10 minutes and no more than 30 days from the request
- safe defaults: `AUTOMATION_ENABLED=false`, `PUBLISH_MODE=dry_run`
- real mode: `AUTOMATION_ENABLED=true`, `PUBLISH_MODE=facebook_schedule`

The application does not retry writes. If it reports an unknown outcome, inspect Meta Business Suite before taking any further action.
...
- [Meta Pages posts guide](https://developers.facebook.com/docs/pages-api/posts/)
